...
def start_crawling(userName, userCellPhoneNumber, kakaoStory_USERURL, naverblog_USERURL, facebook_USERURL, reqClient, fromDate, toDate):
    ...
    if facebook_USERURL is not None:
        try:
            ...
            if ResultDict1['trueOrFalse'] == True:
                print('페이스북 크롤링 성공, Score : ', ResultDict1['tcmScore']['fb_TSCORE'], ResultDict1['tcmScore']['fb_CSCORE'], ResultDict1['tcmScore']['fb_MSCORE'])
    ...
    if kakaoStory_USERURL is not None:
        ...
        try:
            returnValue_kakaostory = kakaostoryScrapBot.crawling_singleData_KakaoStoryCrawlerBot(kakaoStory_USERURL, userName, userCellPhNum)
...
if __name__ == '__main__':
    ...
    while roll < 10000:
        ...
        print('search log index로 가져온 search_log_index값 :', str(search_idx_tbl[0]).replace(" ", ""), ', ', str(search_idx_tbl[1]).replace(" ", ""))
        ...
        if int(str(search_idx_queue[0]).replace(" ", "")) <= int(str(search_idx_tbl[0]).replace(" ", "")):
            ...
            start_crawling(userRealName, userCellPhNum, kakaoStoryURL, naverblogURL, facebookURL, reqClient, startDate, endDate)
            roll = roll + 1
