...
def login_facebook_admin(admin_id, admin_pass):
...
def getTrustScore(fb_pageID, UserName, loginCount, web_driver):
    ...
    loginResult = login_facebook_admin('01027746254', 'Gkstkddbs3#')

    if loginResult == True:
        timeLine_url = 'https://www.facebook.com/' + fb_pageID
        tScoreResult = getProfileData(fb_pageID, timeLine_url, UserName, loginCount, web_driver)

        print(tScoreResult)
    else:
        login_facebook_admin('01027746254', 'Gkstkddbs3#')
...
def getProfileData(fbPageID, User_timeLine_site_url_addr, insertedName, lgnCnt, driver):
    ...
    aboutDictionaryTypeData = getAboutInfoData(fbPageID, lgnCnt, driver)
...
getTrustScore(facebook_USERURL, userName, 1, driver)