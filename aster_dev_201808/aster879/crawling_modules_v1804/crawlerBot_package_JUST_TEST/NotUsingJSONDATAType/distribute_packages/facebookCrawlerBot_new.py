...
def getProfileData(User_timeLine_site_url_addr, insertedName, driver):
    ...
    if user_name == usernamefromDirect:
        ...
        logger.debug('[이름 일치]facebook user name => {} , 이력서 상의 이름 => {}'.format(usernamefromDirect))
        print('이력서 상의 이름 :', usernamefromDirect)
        #field : userNm_resume
    else:
        #print('페이스북 사용자 이름과 이력서의 신청인 이름이 일치하지 않습니다.')
        logger.debug('[이름 불-일치]facebook user name => {} , 이력서 상의 이름 => {}'.format(usernamefromDirect))
        print('이력서 상의 이름 :', usernamefromDirect)
        # field : userNm_resume
    ...
    try:
        ...
        if not intro_text_title:
            ...
            try:
                ...
                print('소개글 게시판 HTML 구조가 변경되어 다시 검색합니다.')
            ...
            print('타임라인 프로필 내 소개글 :', profileDic)
            #field : user_intro
    ...
    try:
        ...
        if lengthOfProfileList == 0:
            ...
            print('공개된 프로필 영역 개수 : ', len(profile_list_test))
            #field : user_profile_length
        ...
        if not profile_list_detail_text_01:
            ...
            print('프로필 정보 : ', profile_list_detail_text_01[0].text)
    ...
    logger.debug('공개된 프로필 : {}'. format(profileDic))
    print('공개된 프로필 :', profileDic)
    #field : user_profile
