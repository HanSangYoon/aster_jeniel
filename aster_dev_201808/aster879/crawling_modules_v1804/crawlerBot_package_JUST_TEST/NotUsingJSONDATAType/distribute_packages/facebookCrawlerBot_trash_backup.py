...
fileHandler = logging.handlers.RotatingFileHandler(r'C://Project_PYTHON3/Aster879Project_package/crawlerBot_package_TENSPACE/Log_Text_Folder/log/' + hereWork + '_crawlerbot_logging_' + currTime, maxBytes=file_max_bytes, backupCount=10, encoding = "UTF-8")
...
def login_facebook(loginCnt, userFacebookPageId, insertedUserName):
    ...
    driver_chrome = r"C:\Project_PYTHON3\chromedriver.exe"
    ...
    returnedValue_from_method = profileTextDataCrawling(login_or_not, loginCnt, user_facebook_page_id, directlyTypedUserName, driver)
...
def profileTextDataCrawling(loginValue, lgnCnt, insertedUser_fbpage_id, insertedUserName, driver):
    ...
    returnedResultDict = getDetailInfoDictionaryType(user_fbpage_id, driver, lgnCnt, insertedUserName)
    ...
    try:
        ...
        if not intro_text_title:
            ...
            try:
                ...
                returnedResultDict['introText'] = intro_text_detail[0].text.replace(' ', '')
            ...
            print('1차 dictionary 결과물 출력 -> ', returnedResultDict)
    ...
    # T_SCORE_01
    try:
        if '남성' in returnedResultDict['성별']:
            #t_score_count_detail = 0
            print('성별이 남성이므로 10점 가산됩니다.')

            print('가산 근거 : 남성일 경우 근속 기간이 여성보다 길기 때문에 10점이 가산 됩니다.')
            t_score_count_detail += 10
            t_score_count += t_score_count_detail

        elif '여성' in returnedResultDict['성별']:
            #t_score_count_detail = 0
            print('성별이 여성이므로 5점 가산됩니다.')

            print('가산 근거 : 여성일 경우 근속 기간이 남성보다 짧기 때문에 5점이 가산 됩니다.')
    ...
    try:
        # try No.1 : 지역관련 정보 검색
        if '근무' in DictionaryValue_list:
            print('근무지 정보 검색 중...')
            t_score_count_detail = 0
            # print("%%", t_score_count)

            if '서울' in DictionaryValue_list:
                print('가산 근거 : 근무지가 서울일 경우 10점이 가산 됩니다.')
                t_score_count_detail += 10
                t_score_count += t_score_count_detail
                # print("%%", t_score_count)
            elif '경기' in DictionaryValue_list:
                print('가산 근거 : 근무지가 경기일 경우 5점이 가산됩니다.')
                t_score_count_detail += 5
                t_score_count += t_score_count_detail
                # print("%%", t_score_count)
            else:
                print('가산 근거 : 근무지가 비-수도권일 경우 3점이 가산됩니다.')
                ...
                if '졸업' in search_t:
                    if '대학원' in search_t:
                        ...
                        if '서울대' or '중앙대' or '덕성여' or '서울교육대' or '홍익대' or '이화여' or '서울시립대' or '동국대' or '서울여' or '연세대' or '명지대' or '숙명여' or '고려대' or '상명대' or '동덕여' or '서강대' or '삼육대' or '국민대' or '서울과학기술대' or '한국체육대' or '성신여' or '한국외' or '숭실대' or '총신대' or '세종대' or '한국종합예술' or '한성대' or '서경대' or '성공회대' in search_t:
                            print('가산 근거 : 출신 대학원 소재지가 서울일 경우 10점이 가산 됩니다.')
                            t_score_count_detail += 10
                            t_score_count += t_score_count_detail
                            # print("%%", t_score_count)
                        else:
                            print('가산 근거 : 출신 대학원 소재지가 경기일 경우 5점이 가산됩니다.')
                            t_score_count_detail += 5
                            t_score_count += t_score_count_detail
                            # print("%%", t_score_count)
                        ...
                        if '서울대' or '중앙대' or '덕성여' or '서울교육대' or '홍익대' or '이화여' or '서울시립대' or '동국대' or '서울여' or '연세대' or '명지대' or '숙명여' or '고려대' or '상명대' or '동덕여' or '서강대' or '삼육대' or '국민대' or '서울과학기술대' or '한국체육대' or '성신여' or '한국외' or '숭실대' or '총신대' or '세종대' or '한국종합예술' or '한성대' or '서경대' or '성공회대' in search_t:
                            print('가산 근거 : 출신 대학교 소재지가 서울일 경우 10점이 가산 됩니다.')
                            t_score_count_detail += 10
                            t_score_count += t_score_count_detail
                            # print("%%", t_score_count)
                        else:
                            print('가산 근거 : 출신 대학교 소재지가 경기일 경우 5점이 가산됩니다.')
                            t_score_count_detail += 5
                            t_score_count += t_score_count_detail
                            # print("%%", t_score_count)
                        ...
                        if '서울' in search_t:
                            print('가산 근거 : 출신 고등학교 소재지가 서울일 경우 10점이 가산 됩니다.')
                            t_score_count_detail += 10
                            t_score_count += t_score_count_detail
                            # print("%%", t_score_count)
                        elif '경기' in search_t:
                            print('가산 근거 : 출신 고등학교 소재지가 경기일 경우 5점이 가산됩니다.')
                            t_score_count_detail += 5
                            t_score_count += t_score_count_detail
                            # print("%%", t_score_count)
                        else:
                            print('가산 근거 : 출신 고등학교 소재지가 비-수도권일 경우 3점이 가산됩니다.')
                            t_score_count_detail += 3
                            t_score_count += t_score_count_detail
                            # print("%%", t_score_count)
                    else:
                        ...
                        if '서울' in search_t:
                            print('가산 근거 : 출신 대학(2~3년제 대학) 소재지가 서울일 경우 10점이 가산 됩니다.')
                            t_score_count_detail += 10
                            t_score_count += t_score_count_detail
                            # print("%%", t_score_count)
                        elif '경기' in search_t:
                            print('가산 근거 : 출신 대학(2~3년제 대학) 소재지가 경기일 경우 5점이 가산됩니다.')
                            t_score_count_detail += 5
                            t_score_count += t_score_count_detail
                            # print("%%", t_score_count)
                        else:
                            print('가산 근거 : 출신 대학(2~3년제 대학) 소재지가 비-수도권일 경우 3점이 가산됩니다.')
                            t_score_count_detail += 3
                            t_score_count += t_score_count_detail
                            # print("%%", t_score_count)
                else:
...
def getDetailInfoDictionaryType(userPageId, driver, loginCnt, userName):
...
def TCMCountGen(tScoreCount, cScoreCount, ResultDict, user_fbpage_url, driver):
    ...
    with open("C:\python_project\just_project\PycharmProjects\CrawledData_"+ hereWork +"_" + currDate + ".csv",
            "w", newline='') as fbFile:
