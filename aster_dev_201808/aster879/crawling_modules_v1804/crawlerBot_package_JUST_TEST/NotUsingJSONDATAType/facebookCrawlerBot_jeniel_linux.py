...
fileHandler = logging.handlers.RotatingFileHandler(
    ...
    '/home/syhan83/dev_tenspace/PycharmProjects/log/' + hereWork + 'crawlerbot_logging_' + currTime, maxBytes=file_max_bytes, backupCount=10)
...
def autoScrollerContentsPhotoText(url_addr, driver):
    ...
    try:
        likeCnt_int = 0
        commentCont_int = 0
        for lngth in range(len(
                detail_fb_info_soup.select('#pagelet_timeline_medley_photos > div:nth-of-type(2) > div > ul > li'))):

            try:
                likeCnt_str = detail_fb_info_soup.select(
...
def readCSV_goodExpressions():
    ...
    reader = csv.reader(
        ...
        open(r'/home/syhan83/dev_tenspace/PycharmProjects/aster_dev_201808/긍정어4.csv', 'rt', encoding='utf-8-sig', newline=''),
        delimiter=' ', quotechar='|')
...
def profileTextDataCrawling(loginValue, lgnCnt, insertedUser_fbpage_id, insertedUserName, driver, requestClient):
    ...
    returnedResultDict['페이스북게시글등록시간'] = ''

    returnedResultDict['해당월게시글개수'] = 0
    returnedResultDict['전월게시글개수'] = 0
        ...
        if len(articleCnt) is not 0:
            ...
            for articleLength in range(len(articleCnt)):
                ...
                for innerArtclLnth in range(len(innerArticleCnt)):
                    ...
                    try:
                        # 게시글 등록 일
                        articleUpdatedTimedays = '%02d' % int(
                            updatedTime.replace(" ", "").split("오")[0].split('월')[1].replace("일", ""))
                    ...
                    # print(updatedTime)
                    returnedResultDict['페이스북게시글등록시간'] = updatedTime
            ...
            returnedResultDict['해당월게시글개수'] = regArticleDateCnt
            returnedResultDict['전월게시글개수'] = regArticleDateCnt2
    ...
    returnedValue_from_method_TCMCountGen = TCMCountGen(t_score_count, c_score_count, returnedResultDict,
                                                        User_timeLine_site_url_addr, driver, requestClient)
...
def readCSV(searchTValue):
    # C:\python_project\aster879_project\PycharmProjects
    reader = csv.reader(
        open('/home/syhan83/dev_tenspace/PycharmProjects/aster_dev_201808/aster879/1_500Corp.csv', 'rt', encoding='utf-8-sig',
