...
import PycharmProjects.crawling_modules_v1804.crawlerBot_package_JUST_TEST.NotUsingJSONDATAType.worknet_just.dbConnectionTest as mySQL_conn
#import PycharmProjects.crawling_modules_v1804.crawlerBot_package_JUST_TEST.NotUsingJSONDATAType.worknet_just.dbConnectionTest as mySQL_conn_test
...
fileHandler = logging.handlers.RotatingFileHandler('C:/dev_syhan/log/' + hereWork + '_crawlerbot_logging_' + currTime, maxBytes=file_max_bytes, encoding='UTF-8', backupCount=10)
...
def getPagination(driver, searchURL):
    ...
    dept_page_url = [page_url.format(i) for i in range(1, 2)]  #테스트를 위해 10페이지로 제한.

    result_workNetDictionaty = {}

    #기존 구인인증 번호 값 추출
    #수집한 구인인증번호값은 현재까지 insert 된 데이터의 구인인증번호 값임.
    database_connection = mySQL_conn.DatabaseConnection_origin()
    database_connection_result = database_connection.conn()

    if database_connection_result == True:
        returnedselectDataList = database_connection.select_record()
    else:
        print('DB 연결이 실패했습니다. 재가동하시기 바랍니다.')
        driver.close()
...
def login_worknet(driver):
    ...
    # 한상윤 계정
    user_id = 'microscope83'
    user_pass = 'Gkstkddbs1!'

    # id and password
    driver.find_element_by_id('custId').send_keys(user_id)
    driver.find_element_by_id('pwd').send_keys(user_pass)
...
def startWorknetCrawling():
    with requests.Session() as s:
        ...
        driver_chrome = r"C:\dev_syhan\chromedriver.exe"
        ...
        '''
        try:
            loginReturn = login_worknet(driver)
        ...
        if loginReturn == True:
            ...
            print('로그인 없이 크롤링 진행합니다.')
            logger.debug('로그인 없이 크롤링 진행합니다.')
